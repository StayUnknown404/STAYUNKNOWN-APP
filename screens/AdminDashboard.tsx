...
function CollectionsAdmin(){
 ...
 if(screen==='editor')return <View>
   ...
   <View style={styles.editor}>
    ...
    <Text style={styles.label}>{editing?'PRODUCTS IN COLLECTION · SELECTED FIRST':'ALL PRODUCTS'}</Text><View style={styles.productPicker}><ScrollView nestedScrollEnabled showsVerticalScrollIndicator>{products.filter(p=>selectedIds.includes(String(p.id))).map(p=><Pressable key={`selected-${p.id}`} style={[styles.pickRow,styles.pickSelected]} onPress={()=>setSelectedIds(ids=>ids.filter(id=>id!==String(p.id)))}><Text style={[styles.pickName,styles.pickNameSelected]}>{p.name}</Text><Text style={[styles.pickMark,styles.pickMarkSelected]}>✓</Text></Pressable>)}{products.filter(p=>!selectedIds.includes(String(p.id))).map(p=><Pressable key={`available-${p.id}`} style={styles.pickRow} onPress={()=>setSelectedIds(ids=>[...ids,String(p.id)])}><Text style={styles.pickName}>{p.name}</Text><Text style={styles.pickMark}></Text></Pressable>)}</ScrollView></View>
...
function PromosAdmin(){
 ...
 if(screen==='editor')return <View>
   ...
   <View style={styles.editor}>
    ...
    {scope==='product'?<View style={styles.productPicker}><ScrollView nestedScrollEnabled showsVerticalScrollIndicator>{products.filter((item:any)=>targetIds.includes(String(item.id))).map((item:any)=><Pressable key={`selected-${item.id}`} style={[styles.pickRow,styles.pickSelected]} onPress={()=>toggleTarget(String(item.id))}><Text style={[styles.pickName,styles.pickNameSelected]}>{item.name||item.id}</Text><Text style={[styles.pickMark,styles.pickMarkSelected]}>✓</Text></Pressable>)}{products.filter((item:any)=>!targetIds.includes(String(item.id))).map((item:any)=><Pressable key={`available-${item.id}`} style={styles.pickRow} onPress={()=>toggleTarget(String(item.id))}><Text style={styles.pickName}>{item.name||item.id}</Text><Text style={styles.pickMark}></Text></Pressable>)}</ScrollView></View>:null}
    {scope==='collection'?<View style={styles.productPicker}><ScrollView nestedScrollEnabled showsVerticalScrollIndicator>{collections.filter((item:any)=>targetIds.includes(String(item.id))).map((item:any)=><Pressable key={`selected-${item.id}`} style={[styles.pickRow,styles.pickSelected]} onPress={()=>toggleTarget(String(item.id))}><Text style={[styles.pickName,styles.pickNameSelected]}>{item.name||item.id}</Text><Text style={[styles.pickMark,styles.pickMarkSelected]}>✓</Text></Pressable>)}{collections.filter((item:any)=>!targetIds.includes(String(item.id))).map((item:any)=><Pressable key={`available-${item.id}`} style={styles.pickRow} onPress={()=>toggleTarget(String(item.id))}><Text style={styles.pickName}>{item.name||item.id}</Text><Text style={styles.pickMark}></Text></Pressable>)}</ScrollView></View>:null}
...
function NotificationsOrSupport({mode}:{mode:'support'|'notifications'}){
 ...
 async function send(t:any){if(!reply[t.id]?.trim())return;try{await replyToSupportTicket(t.id,reply[t.id].trim());setReply(r=>({...r,[t.id]:''}));const fresh=await getAdminSupport();setList(fresh);const updated=fresh.find(x=>String(x.id)===String(t.id));if(updated)setSelected(updated);Alert.alert('Sent','Reply sent.');}catch(e:any){Alert.alert('Reply failed',e?.message||'');}}
 async function close(t:any){try{const next=(String(t.status||'OPEN').toUpperCase()==='CLOSED'?'OPEN':'CLOSED');await updateSupportStatus(t.id,next);const fresh=await getAdminSupport();setList(fresh);const updated=fresh.find(x=>String(x.id)===String(t.id));if(updated)setSelected(updated);Alert.alert('Saved',next==='CLOSED'?'Ticket closed.':'Ticket reopened.');}catch(e:any){Alert.alert('Status update failed',e?.message||'');}}
 ...
 if(mode==='support'){
   if(selected)return <View>
     <View style={styles.editorHeader}><Pressable onPress={()=>setSelected(null)} style={styles.smallButton}><Text style={styles.smallText}>← BACK</Text></Pressable><Text style={styles.editorPageTitle}>SUPPORT TICKET</Text><View style={styles.headerSpacer}/></View>
     <View style={styles.card}><Text style={styles.rowTitle}>{selected.subject||'SUPPORT TICKET'}</Text><Text style={styles.rowMeta}>{selected.email||selected.userEmail||selected.userId||'Customer'}</Text>{selected.orderId||selected.orderNumber?<Text style={styles.rowMeta}>Order: {selected.orderNumber||selected.orderId}</Text>:null}<View style={styles.statusRow}><Text style={styles.statusBadge}>{String(selected.status||'OPEN').toUpperCase()}</Text><Pressable style={styles.smallButton} onPress={()=>void close(selected)}><Text style={styles.smallText}>{String(selected.status||'OPEN').toUpperCase()==='CLOSED'?'REOPEN TICKET':'CLOSE TICKET'}</Text></Pressable></View></View>
     <ScrollView style={styles.supportChat} contentContainerStyle={styles.supportChatContent}>{selected.message||selected.body?<View style={[styles.chatBubble,styles.theirsBubble]}><Text style={styles.chatFrom}>CUSTOMER</Text><Text style={styles.reply}>{selected.message||selected.body}</Text></View>:null}{Array.isArray(selected.messages)&&selected.messages.map((r:any,i:number)=>{const mine=String(r.from||r.sender||'').toLowerCase().includes('admin')||String(r.from||r.sender||'').toLowerCase().includes('support');return <View key={'m'+i} style={[styles.chatBubble,mine?styles.mineBubble:styles.theirsBubble]}><Text style={styles.chatFrom}>{mine?'YOU':(r.from||r.sender||'CUSTOMER')}</Text><Text style={styles.reply}>{r.message||r.text||''}</Text></View>})}{Array.isArray(selected.replies)&&selected.replies.map((r:any,i:number)=><View key={'r'+i} style={[styles.chatBubble,styles.mineBubble]}><Text style={styles.chatFrom}>YOU</Text><Text style={styles.reply}>{r.message||r.text||''}</Text></View>)}</ScrollView>
     <TextInput style={styles.input} value={reply[selected.id]||''} onChangeText={v=>setReply(r=>({...r,[selected.id]:v}))} placeholder="Reply to customer" placeholderTextColor="#555" multiline/>
     <Pressable style={styles.white} onPress={()=>void send(selected)}><Text style={styles.black}>SEND REPLY</Text></Pressable>
   </View>;
   return <View><View style={styles.sectionHeader}><Text style={styles.sectionTitle}>SUPPORT · {list.length}</Text><Pressable onPress={()=>void load()}><Text style={styles.add}>REFRESH</Text></Pressable></View>{list.length===0?<Text style={styles.muted}>Nothing here.</Text>:list.map(item=><Pressable key={item.id} style={styles.row} onPress={()=>setSelected(item)}><Text style={styles.rowTitle}>{item.subject||'SUPPORT TICKET'}</Text><Text style={styles.rowMeta}>{item.email||item.userEmail||item.userId||'Customer'} · {String(item.status||'OPEN').toUpperCase()}</Text>{item.orderId||item.orderNumber?<Text style={styles.rowMeta}>Order: {item.orderNumber||item.orderId}</Text>:null}<Text style={styles.rowMeta} numberOfLines={2}>{item.message||item.body||'Open ticket →'}</Text><Text style={styles.openTicket}>OPEN TICKET →</Text></Pressable>)}</View>;
 }
