...
export default function Support({ onBack, initialTicketId }: Props) {
  ...
  useEffect(() => { void load(); }, []);
  useEffect(() => { if (initialTicketId && tickets.length) { const found = tickets.find(t => String(t.id) === String(initialTicketId)); if (found) setSelected(found); } }, [initialTicketId, tickets.length]);
      ...
      <SafeAreaView style={styles.container}>
        <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={8}>
          <View style={styles.chatHeader}>
            <Pressable style={styles.backButton} onPress={() => setSelected(null)}><Text style={styles.backText}>←  BACK</Text></Pressable>
